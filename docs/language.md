...
Peony executes a defined **Python 3.12 subset**. This page specifies the supported syntax and core objects, the behavior of important edges, and the compatibility boundary. It applies equally to the native executable and the Web Worker distribution. The [architecture](architecture.md) explains the implementation, [libraries](libraries.md) covers imports, the [native CLI](native-cli.md) covers process hosting, and [browser embedding](embedding.md) covers the JavaScript host.
...
Imports resolve registered Zig native modules and `.py` user modules or packages stored in the VFS. Native names have precedence when first resolved. User modules are searched under `/home`, `/course`, then `/tmp`; package source is `__init__.py`. `sys.modules` is the active run's cache. A host can mount Python source as a VFS file, but Peony ships no Python-source implementation of its own libraries. Consecutive browser `run()` calls use new globals and import caches while keeping `/course` and `/home` files; `session.reset()` clears the session, including those files. The native executable owns one run and one process-local VFS.

## Explicit compatibility boundary

Peony does not claim full Python 3.12 or CPython behavior. The main excluded families are non-UTF-8 source encodings, non-ASCII identifiers, complex numbers, async syntax/runtime, `yield from`, exception groups/`except*`, unrestricted structural patterns, PEP 695 type syntax, dynamic code execution, custom metaclasses and full descriptors, user finalizers, and unlisted standard-library APIs. Network policy is supplied by the active host. Neither Python code nor the `ssl` compatibility object can replace browser or native TLS policy; browser execution also remains subject to CORS.
