...
`zig build wasm` installs `zig-out/bin/peony.wasm`. Build output and compiler caches stay ignored. The Node ABI suite exercises that artifact directly through WebAssembly, from bytes and with `instantiateStreaming()` using `application/wasm`.
