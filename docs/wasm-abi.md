...
The current compiler executes scalar expressions and assignments, `if`/`elif`/`else`, `while`/`else`, `for`/`else`, short-circuit Boolean operations, comparisons, and lazy integer `range(...)` and Unicode string iteration. It supports ordinary `def` functions, explicit and implicit returns, recursion, nested calls, mutable lexical closures, `global`/`nonlocal`, positional-only and keyword-only parameters, defaults, annotations evaluated at definition time, and direct keyword argument binding. `print` and `range` are first-class callable builtins; `print` supports `sep` and `end`. Calls preserve left-to-right evaluation and source-positioned Python exceptions across WASM timeslices. Variadic capture, call unpacking, tuple/unpacking loop targets, container iteration, classes, generators, events and host resume packets remain outside this subset and return `UNSUPPORTED` with a diagnostic when recognized during compilation. `assert` and `del` also remain explicitly unsupported pending their planned milestones.
